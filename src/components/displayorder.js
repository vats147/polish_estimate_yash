...
const ManageColor = ({ onClose, initialValue, mode }) => {
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === "edit") {
        const response = await axios.put('http://localhost:8080/updatecolor', { orderId: initialValue.orderId, ...data });
...
const ColorList = () => {
  ...
  const fetchColors = async () => {
    try {
      const response = await axios.get("http://localhost:8080/getallorder");

...
const ManageShape = ({ onClose, initialValue, mode }) => {
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === "edit") {
        const response = await axios.put('https://diamonddemo-backend.vercel.app/updateshape', { shape_id: initialValue.shape_id, ...data });
        toast.success('Shape updated successfully!');
      } else {
        const response = await axios.post('https://diamonddemo-backend.vercel.app/addshape', data);
...
const ShapeList = () => {
  ...
  const fetchShapes = async () => {
    try {
      const response = await axios.get("https://diamonddemo-backend.vercel.app/getallshape");
  ...
  const handleConfirmAction = async () => {
    if (modalType === "delete") {
      try {
        await axios.delete(`https://diamonddemo-backend.vercel.app/deleteshape`, {
